...
session_start();
	...
	define(CONTACT_FAIL,		'signup-mailbox-page3.php');
	...
	$email_from = 'contact@csnotepad.co.uk';
        ...
        if(!$captcha){
			header("Location: signup-mailbox-page3.php?CaptchaFail=True#recaptchaerror");
	...
	if($accountName === '') {
		$_SESSION['user-feedback']['errors']['accountName'] = 'Please state your first name';
	...
	if($_SESSION['user-feedback']['success']) {
		...
		emailUser($mail, $email, $file, $values, array(
			'accountName' => $accountName,
		));
	}
	...
	function emailUser($mail, $email, $file) {
		...
		$mail->Body= <<<HTML
			<!-- <body bgcolor="#777777" style="background: #777777;"> -->
			<body>
	...
	function emailClient($mail, $values) {
		...
		$mail->Body= <<<HTML
			<body bgcolor="#f2f2f2" style="background: #f2f2f2;">
				<table width="500" align="center" cellpadding="0" cellspacing="10" bgcolor="#ffffff" style="background: #ffffff;">
					<tr><td colspan="2">You have received a request to set up a mailbox.</td></tr>
					<tr><td colspan="2">&nbsp;</td></tr>
					<tr><td><strong style="font-weight: bold;">Account Holder Name:</strong></td><td>{$values['accountName']}</td></tr>
					...
					<tr><td><strong style="font-weight: bold;">Term:</strong></td><td>{$values['term']}</td></tr>
					<tr><td><strong style="font-weight: bold;">Service:</strong></td><td>{$values['service']}</td></tr>
					<tr><td><strong style="font-weight: bold;">Account Address:</strong></td><td>{$values['acAddr']}</td></tr>
					<tr><td><strong style="font-weight: bold;">Postcode:</strong></td><td>{$values['acPcode']}</td></tr>
					<tr><td><strong style="font-weight: bold;">Conf Email:</strong></td><td>{$values['confEmail']}</td></tr>
					<tr><td><strong style="font-weight: bold;">Came from:</strong></td><td>{$_SERVER['HTTP_REFERER']}</td></tr>
				</table>
			</body>
HTML;
