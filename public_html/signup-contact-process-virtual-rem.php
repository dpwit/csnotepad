...
session_start();
	...
	define(CONTACT_FAIL,		'signup-virtual-rem.php');
	...
	$email_from = 'contact@csnotepad.co.uk';
        ...
        if(!$captcha){
			header("Location: signup-virtual-rem.php?CaptchaFail=True#recaptchaerror");
	...
	switch($service) {
		...
		case 'Virtual address':
			$file['name'] = 'CSnotepad virtual office address information pack rem.pdf';
			$file['path'] = dirname(__FILE__) . '/html/pages/signup_process/CSnotepad virtual office address information pack rem.pdf';
			break;
		default:
