

			<div id="mainSection">
				
				<div id="mainSection2ColLeft" class="productContent">
			
					<h2>
						<?=$item->label?>
					</h2>
					<?=$item->text?>
			    </div>
				<div id="mainSection2ColRight">
					<div id="sideBarBoxTop">
						
					</div>
					<div id="sideBarBoxMiddle">
						<?php
							include BASEPATH.'../includes/testimonials.php';
						?>
				  </div>
					<p>
						<a href="/reasons" title="Why we're better than letting calls go to voicemail">
							<img width="217" height="75" alt="Why we're better than letting calls go to voicemail" src="images/answer.jpg" title="Why we're better than letting calls go to voicemail" ></a>
						<a href="/Temp" title="Why we're better than employing extra staff">
							<img width="217" height="75" alt="Why we're better than employing extra staff" src="images/temp.jpg" title="Why we're better than employing extra staff" ></a> 
						<a href="/competitors" title="Why we're better than the competition">
							<img width="217" height="75" alt="Why we're better than the competition" src="images/competition.jpg" title="Why we're better than the competition" ></a>
					</p>
				<br>
</div>
				<div class="clear"></div>
				
			</div>