<?php
			...
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url = "http://api.twitter.com/1/users/show/$twitter_id.json");
			//var_dump($url);
			...
			$twitter_name = explode(' ', $twitter_name);
			$twitter_name = $twitter_name[0];
					...
					<div class="thinBorder twitter box">
						<?php 
							if(!$_SESSION['label_uid']) {
								?><h2 class="twitter_<?=$twitter_screen_name?>"><a href="http://twitter.com/<?=$twitter_screen_name?>"><?=$twitter_name?></a> on Twitter</h2><?php 
							}else { 
