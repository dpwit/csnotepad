...
session_start();
	...
	define(CONTACT_SUCCESS,		'/test-site/csnotepad-agreement-ta.php');
	define(CONTACT_FAIL,		'/test-site/setup-questions-ta.php');
	...
	$email_from = 'contact@csnotepad.co.uk';
        ...
        if(!$captcha){
			header("Location: /test-site/setup-questions-ta.php?CaptchaFail=True#recaptchaerror");
	...
	function emailUser($mail, $email, $file) {
		...
		$mail->Subject = 'CSnotepad - Setup questions received';
		$mail->ClearAddresses();
									...
									<tr>
										<td>
											<table style="font-size: 14px; font-family: Arial" cellspacing="5">
												...
												<tr>
													<td>
														Thank you for completely the required setup questions
														<br>
														<br>
														<br>
														Should we have any questions, we'll be in contact.
													</td>
												</tr>
												<tr>
													<td>
	...
	function emailClient($mail, $values) {
		...
		$mail->Body= <<<HTML
			<body bgcolor="#f2f2f2" style="background: #f2f2f2;">
				<table width="500" align="center" cellpadding="0" cellspacing="10" bgcolor="#ffffff" style="background: #ffffff;">
					<tr><td colspan="2">You have received some setup questions.</td></tr>
					<tr><td colspan="2">&nbsp;</td></tr>
					<tr><td><strong style="font-weight: bold;">Business name:</strong></td><td>{$values['businessName']}</td></tr>
					<tr><td><strong style="font-weight: bold;">Contact name:</strong></td><td>{$values['contactName']}</td></tr>
...
HTML;
	...
	header('location: /test-site/csnotepad-agreement-ta.php');
