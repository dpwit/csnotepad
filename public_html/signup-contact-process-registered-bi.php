...
session_start();
	...
	define(CONTACT_FAIL,		'signup-registered-bi.php');
	...
	$email_from = 'contact@csnotepad.co.uk';
        ...
        if(!$captcha){
			header("Location: signup-registered-bi.php?CaptchaFail=True#recaptchaerror");
	...
	switch($service) {
		...
		case 'Registered office':
			$file['name'] = 'CSnotepad virtual office address information pack ga.pdf';
			$file['path'] = dirname(__FILE__) . '/html/pages/signup_process/CSnotepad virtual office address information pack ga.pdf';
			break;
		default:
