...
session_start();
	...
	define(CONTACT_FAIL,		'signup-telephone-bi.php');
	...
	$email_from = 'contact@csnotepad.co.uk';
        ...
        if(!$captcha){
			header("Location: signup-telephone-bi.php?CaptchaFail=True#recaptchaerror");
	...
	switch($service) {
		case 'Telephone':
			$file['name'] = 'CSnotepad telephone answering information pack bi.pdf';
			$file['path'] = dirname(__FILE__) . '/html/pages/signup_process/CSnotepad telephone answering information pack bi.pdf';
			break;
		case 'Order taking':
