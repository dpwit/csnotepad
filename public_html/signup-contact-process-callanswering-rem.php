...
session_start();
	...
	define(CONTACT_FAIL,		'signup-callanswering-rem.php');
	...
	$email_from = 'contact@csnotepad.co.uk';
        ...
        if(!$captcha){
			header("Location: signup-callanswering-rem.php?CaptchaFail=True#recaptchaerror");
	...
	switch($service) {
		case 'Callanswering':
			$file['name'] = 'CSnotepad call answering information pack rem.pdf';
			$file['path'] = dirname(__FILE__) . '/html/pages/signup_process/CSnotepad call answering information pack rem.pdf';
			break;
		case 'Order taking':
