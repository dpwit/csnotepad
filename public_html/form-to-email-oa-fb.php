...
session_start();
	...
	define(CONTACT_FAIL,		'signup-office-address-fullform-fb.php');
	//$mailto = 'contactus@csnotepad.co.uk';
	//$copyto = 'jim@bozboz.co.uk, webcontact@bozboz.co.uk';
	$mailto = 'rdonald@csnotepad.co.uk, wfrancis@csnotepad.co.uk, tlownds@csnotepad.co.uk, rcross@csnotepad.co.uk';
	$mailto_bcc = 'darren.whatford@dpwit.co.uk';
	$email_from = 'contact@csnotepad.co.uk';
        ...
        if(!$captcha){
			header("Location: signup-office-address-fullform-fb.php?CaptchaFail=True#recaptchaerror");
	...
	function emailUser($mail, $email, $file) {
		...
		$mail->Subject = 'CSnotepad Virtual Office Address setup';
		$mail->ClearAddresses();
	...
	function emailClient($mail, $values) {
		...
		$mail->Body= <<<HTML
			<body bgcolor="#f2f2f2" style="background: #f2f2f2;">
				<table width="500" align="center" cellpadding="0" cellspacing="10" bgcolor="#ffffff" style="background: #ffffff;">
					<tr><td colspan="2">You have received a new message via Facebook marketing from {$_POST['accountName']} to set up a new virtual office address.</td></tr>
					...
					<!--<tr><td><strong style="font-weight: bold;">Brighton landline phone number:</strong></td><td>{$_POST['telQuestion']}</td></tr>-->
