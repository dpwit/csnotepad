...
session_start();
	...
	define(CONTACT_FAIL,		'signup-order-rem.php');
	...
	$email_from = 'contact@csnotepad.co.uk';
        ...
        if(!$captcha){
			header("Location: signup-order-rem.php?CaptchaFail=True#recaptchaerror");
	...
	switch($service) {
		case 'Telephone':
			...
			break;
		case 'Order taking':
			$file['name'] = 'CSnotepad order taking information pack-rem.pdf';
			$file['path'] = dirname(__FILE__) . '/html/pages/signup_process/CSnotepad order taking information pack-rem.pdf';
			break;
