...
class DbList {
	function DbList() {
		...
		$this->methodTable = array(
			...
			"getDbaseTables" => array(
				"description" => "Read whole Database",
				"access" => "remote"
			)
	...
	// readTable requests results, passes them back as a resultset which
	// comes back as an instance of mx.remoting.RecordSet
	
	
	function readTable($start, $limit) {
		// Create SQL query
		$sql = sprintf("SELECT * FROM links LIMIT %d, %d", $start, $limit);
		// Trace the query in the NetConnection debugger
		NetDebug::trace($sql);
		// Run query on database
		$result = mysql_query($sql);
