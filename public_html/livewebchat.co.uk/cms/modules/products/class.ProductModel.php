<?
	...
	class ProductModel extends MP3StoreModel {
		...
		function __construct($obj=null){
			$this->addModelNameForHooks('product');
			...
			$this->hasMM('product_attribute_options',array(
				'composition'=>true,
				'input'=>dirname(__FILE__).'/fields/class.AttributeField.php:AttributeField',
			));
...
//			$this->hasMany('related_products',array('composition'=>true));
		...
		function template($for){
			return "modules/products/".$this->applyFilters('template','product-'.$for,$for).".php";
		}
		function getPurchaseUrl(){
			return "/basket/add/".$this->getId();
		}
		function filter_template($current,$purpose){
			$categories = $this->categories();
			$type = $this->product_type();
			array_unshift($categories,$type);
			$fieldName = "product_".$purpose."_template";
			foreach($categories as $c){
				if($template = @$c->$fieldName){
					$current = $template;
					break;
				}
			}
		...
		function filter_composition_fields($fields,$relationship){
			switch($relationship){
				case 'variations':
					...
					if(!$fields['stock']) unset($fields['stock']);
					if(Config::value('images_for_variations','products')) {
						$fields['product_images'] = $old['product_images'];
					}
		...
		function filter_composition_created($related,$relationship){
			switch($relationship){
				case 'variations':
					$related=$this->createVariation(array('uid'=>$related->uid));
					break;
			}
			return $related;
		}
		...
		function getDisplayPrice(){
			return Config::value('display_ex_vat') ? $this->getPriceExVat() : $this->getPriceIncVat();
		}
		function getPriceIncVat(){
			return $this->getPrice();
		}
		function getPriceExVat(){
			$price = $this->price;
			if(Config::value('use_vat')){
				$vat = Config::value('vat_rate');
				$price = number_format(($price*100/(100+$vat)),2);
			}
			return $price;
		}
...
//			require_once(dirname(__FILE__).'/fields/class.StockByAttributeField.php');
			...
			if(@$fields['variations']){
				if(!($this->product_type()&&$this->product_type()->product_attributes())){
					$fields['variations']->setParam('hidden',true);
				} else {
					$fields['variations']->setParam('label','Product Variations');
				}
			}
		...
		function getFields(){
			...
			if($t = $this->product_type())	
				$fields['product_attribute_options']->setAttributes($t->product_attributes());
			return $fields;
		}
		...
		function create($class,$r,$table){
			$class = $this->getClassForType($class,$r->product_type_uid);
			return parent::create($class,$r,$table);
		}
		function getClassForType($default,$typeId){
			$class = cms_apply_filter('get_product_class',$default,$typeId,Model::g('Product_Type',$typeId));
			if((!class_exists($class)) && ($inst = Model::loadModel($class))){
				$class = get_class($inst);
			}
			return $class;
		}
		function filter_new_class($class,$values){
			$class = $this->getClassForType($class,@$values['product_type_uid']);
			return $class;
		}
		...
		function anyInStock($count=1,$order_uid=0){
			foreach($this->variations() as $v){
				if($v->inStock($count,$order_uid)) return true;
			}
			return $this->inStock($count,$order_uid);
		}
		...
		function isAvailableForSale($count=1,$order_item_uid=0){
			foreach($this->variations() as $v){
				if($v->isAvailableForSale($count,$order_item_uid)) return true;
			}
			return $this->isSelfAvailableForSale($count,$order_item_uid);
		}
		function isSelfAvailableForSale($count=1,$order_item_uid=0){
		...
		function is_attribute_option($key){
			return strpos($key,'product_attribute_options')===0;
		}
		function filter_cms_edit_form($sections){
			...
			$sections['Stock'] = array('price','weight','stock','stock_by_attribute','variations');
			$sections['Info'] = array('description','product_images','products_in','related_products','manufacturers');
			$sections['Info'] = array_merge($sections['Info'],array_filter($keys,array($this,'is_attribute_option')));
...
//			$sections['Parts'] = $parts;
	...
	class Product_Type extends BozModel {
		function __construct($obj=null,$table=false){
			...
			$this->hasMany('children',array('model'=>'Product_Type','ref_field'=>'parent_uid'));
		...
		function filter_fields_built($fields){
			...
			$fields['product_detail_template'] = new Field('product_detail_template');
			$fields['product_sidebar_template'] = new Field('product_sidebar_template');
			return $fields;
		}
		function getBuyText(){
