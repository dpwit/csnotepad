<?
	...
	class ProductHooks {
		function __construct(){
			...
			cms_register_filter('composition_fields',$this);
			cms_register_filter('product_browsing_restrict',$this);
		}

		function product_browsing_restrict($where){
			$where['list_in_shop']=1;
			return $where;
		}

		function composition_fields($fields,$obj){
			unset($fields['uid']);
			return $fields;
		}
		...
		function config_defaults($config){
			...
			$config['products']['featured_products'] = 1;
			$config['products']['allow_multiple_features'] = 0;
			return $config;
		}
		...
		function cms_menu($menu){
			...
			$menu['Catalogue'] = array("View Categories"=>"overview.php?section=Products&model=ProductCategory");
			
			if(Config::value('featured_products','products'))
				$menu['Catalogue']["Featured Products"]="overview.php?section=Products&model=Feature";
				
			if(Config::value('allow_multiple_features','products')) $menu['Catalogue']['New Feature'] = 'newItem.php?model=Feature';
...
//				"New Attribute Option"=>"newItem.php?section=Shop Config&model=product_attribute_option",
		...
		function models_loaded(){
			...
			Model::addModel('FeaturedProducts',dirname(__FILE__).'/class.Feature.php','Feature');
		...
		function cms_header(){
?>
<link rel='stylesheet' type='text/css' href='/cms/modules/products/css/cms.css'/>
<script type='text/javascript' src='/cms/modules/products/css/cms.js'></script>
<?
		}
