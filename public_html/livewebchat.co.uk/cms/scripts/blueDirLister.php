...
//I HAVE MODDE THIS BY COMMENTING OUT LINES circa 428/429

//$log_user = $_POST["log_user"];
//$log_pass = $_POST["log_pass"];

//this enables me to post directly to this script using it to login from elsewhere
...
Directory is now not Hardcoded and needs to be sent in URL encoded fashion

Modified code so that this Dir  location is stored in session. Otherwise it was lost on page refresh

http://ww.blue-edge.co.uk/blueDirLister.php?log_user=mike&log_pass=ninja&action=login&dirVir=old
...
$username = "mike";										    /* The username. Case sensitive. */
$password = "ninja";										/* The password. Case sensitive */
$imageFolder ="images";

$_SESSION['dirVir'] = $dirVir;		
$dir = 	$_SESSION['dirVir']	;								/* The subdir name. This file has to be located 
...
$CSS_fontFamily = "Arial , \"Arial Narrow\", sans-serif";	/* Normal font */
$CSS_bg = "#fff";											/* Page background */
$CSS_fontColour = "#333333";									/* Normal font colour */
...
$CSS_tableHeaderBg = "#FFFFFF";								/* Background colour for the table header */
$CSS_tableHeaderColour = "#003366";							/* Font colour for the table header */
...
$CSS_menuWidth = "150px";									/* The width of the menu */
...
$msg["en"]["title"] = "Blue Edge Document Management";							/* The title in the browser window. */
$msg["en"]["headline"] = "You Are Viewing the Project Folder For: %VAR1%/ ";					/* The headline above the dashed line. */
...
$msg["en"]["tableFile"] = "Directory Name / File Name";								/* Table header for the 'File' column. */
$msg["en"]["tableOptions"] = "Options";							/* Table header for the 'Options column. */
$msg["en"]["tableDate"] = "Last changed";						/* Table header for the 'Last change' column. */
...
// uncommenting these lines allows variables to be posted to the script

//$log_user = $_POST["log_user"];
//$log_pass = $_POST["log_pass"];

//some user variables ?? By mike
	...
	<body>
		...
		?>
			...
			<div>
				...
				<input type="hidden" class="formbox" name="action" value="login" />
				<input type="submit" class="formbox" value="<?php echo printMsg("buttonLogin");?>" />
			</div>
		</form>
	</body>
</html>
...
<form action="<?php echo $formaction;?>" method="post">
	...
	<input class="formbox" type="submit" value="<?php echo printMsg("buttonSaveFile");?>" name="submittype" />
	<input class="formbox" type="reset" name="reset" value="<?php echo printMsg("buttonReset");?>" />
	<input class="formbox" type="submit" value="<?php echo printMsg("buttonCancel");?>" name="submittypecancel" />
	<input class="formbox" type="checkbox" name="convertspaces" id="convertspaces" /><label for="convertspaces"><?php echo printMsg("textConvertToTabs");?></label>
	</fieldset>
</form>
<?php
		} else {
...
<p>Please Contact Blue Edge on 01273 328125 for any support Issues</p>
...
?><?php
	...
	//echo "<h1 style="font-size:12px;" >".printMsg("headline", $dir)."</h1>";
?>
<div style="width:800px; border:5px solid #CCCCCC; margin:10px; padding:10px; margin-left:auto; margin-right:auto;">
<div style="width:800px; height:100px; float:right; border-bottom:5px solid #CCCCCC;margin-top:5px;" >  <img src="html/images/blue_logo_horiz.gif " align="right" /></div>
...
<div id="forms">
	<form action="<?php echo $formaction;?>" method="post" enctype="multipart/form-data" accept="<?php echo outputAcceptedFiles($allowedfile);?>">
		<fieldset>
			...
			<input class="formbox" type="file" name="localfile" id="localfile" />
			<input type="hidden" name="action" value="upload" />
			<input class="formbox" type="submit" value="<?php echo printMsg("buttonUpload");?>" />
		</fieldset>
	</form>
	<form action="<?php echo $formaction;?>" method="post">
		<fieldset>
			<legend>
				<?php echo printMsg("textNew");?> 
				...
				<?php echo printMsg("textNew");?>  <label for="newtypefile">
				<?php echo printMsg("textFile");?></label> 
				<input type="radio" id="newtypefile" name="newtype" value="file" /><br/>

<!--			<input type="radio" id="newtypeurl" name="newtype" value="url" />
				<label for="newtypeurl"><?php echo printMsg("textUrl");?></label>-->
			</legend>
			<input class="formbox" size="30" name="newdir" id="newdir" />
			<input type="hidden" name="action" value="mkdir" />
			<input class="formbox" type="submit" value="<?php echo printMsg("buttonMkdir");?>" />
		</fieldset>
	</form>
</div>
<?php
	...
	if (count($filelist) != 0 || is_array($subdirs)) {
		...
		foreach ($filelist as $file) {
			if (is_dir("{$dir}/{$file}")) {
				...
				//this is where sub directory links are set
					
					$filelink = "<a href=\"{$self}?action=list&amp;subdir={$file}\" title=\"".printMsg("titleListFiles", $file)."\" class=\"dir\"> <img style=\" border:0px ; padding:0px; \" src=\"images/folder.gif\" /> &nbsp; {$file}</a> <span class=\"size small\">({$size})</span>";
				} else {
			...
			if (is_writeable("{$dir}/{$file}")) {
				...
				$rename = "<input type=\"text\" class=\"formbox\" name=\"newfile\" value=\"{$file}\" size=\"".strlen($file)."\" />
							<input type=\"submit\" class=\"formbox\" value=\"".printMsg("buttonOk")."\" />";
				if (IsSet($subdir)) {
				...
				$delete = "..";
				$rename = "<input class=\"formbox\" type=\"text\" name=\"newfile\" disabled=\"disabled\" />
					<input type=\"submit\" class=\"formbox\" value=\"rename\" disabled=\"disabled\" />";
			}
			if ($i % 2 == 0) {
				$style = "white";
			} else {
	...
	echo "<p>Directory Management &copy; <!-- Copyright --> 2006 Blue Edge </a>.All rights reserved.</a></p>";
}
?>
