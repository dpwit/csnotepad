...
session_start();
	...
	define(CONTACT_FAIL,		'signup-telephone-rem.php');
	...
	$email_from = 'contact@csnotepad.co.uk';
        ...
        if(!$captcha){
			header("Location: signup-telephone-rem.php?CaptchaFail=True#recaptchaerror");
	...
	switch($service) {
		case 'Telephone':
			$file['name'] = 'CSnotepad telephone answering information pack rem.pdf';
			$file['path'] = dirname(__FILE__) . '/html/pages/signup_process/CSnotepad telephone answering information pack rem.pdf';
			break;
		case 'Order taking':
